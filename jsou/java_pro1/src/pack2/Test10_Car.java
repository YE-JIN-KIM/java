...
//자동차 설계도(클래스)
public class Test10_Car {  // [접근지정자][기타제한자] class 클래스명 - 무조건 대문자
	int wheel; ///멤버변수(멤버필드, 전역변수, 속성-attribute)
	private int airBag = 1; // [접근지정자][기타제한자] tyoe(g형)- 변수명
	//접근지정자 : default, private, public, protected
	private int speed;  // 접근지정자가 private인 경우 캡슐화
	public String irum = "길동";  
	// int인 경우 0으로 초기화,  double 인 경우 0.0으로 초기화
	
	public Test10_Car() {  //constructor(생성자)
		// 객체 생성시 가장 먼저 수행 : 초기화 담당
		// 클래스와 이름이 같고 반환형이 없는 메소드
		// 반드시 생성자가 있어야 하는데 안에 내용이 없다면 안적어도 설계가능
		wheel = 4;
		speed = 10;
	...
	public void abc() { //멤버 메소드(method)
		...
		def();  //method 호출
		int result = korea(50); //매개변수가 있는 method 호출. 이 때는 인수(Argument)를 줌(매개변수를 인수에 전달해 값을 기억)
